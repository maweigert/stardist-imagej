                    ...
                    bool* result)
{
  ...
  std::cout<<"NMS folks again or what did you say??!!!"<<std::endl;
      ...
      for (int y = std::max(0,bbox[2]); y <= std::min(ny-1,bbox[3]); ++y) {
        for (int x = std::max(0,bbox[4]); x <= std::min(nx-1,bbox[5]); ++x) {
          ...
          if (inside){
            ...
            int new_label_if_labeled = use_overlap_label?overlap_label:result[x+y*nx+z*(nx*ny)];

            result[x+y*nx+z*(nx*ny)] = result[x+y*nx+z*(nx*ny)]==0?labels[i]:new_label_if_labeled;
